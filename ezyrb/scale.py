...
class Scale(object):
    ...
    def method(self, method):
        if method not in self.available_methods:
            raise ValueError(
                '{} not available for {} object.'.format(
                    method, self.__class__.__name__),
                'Please chose one between {}.'.format(' '.join(
    ...
    def standardisation(self, x):
        ...
        """
        return (x - np.mean(x, axis=self.axis, keepdims=True)) / np.std(
            x, axis=self.axis, keepdims=True)

    def mean(self, x):
        return (x - np.mean(x, axis=self.axis, keepdims=True)) / (
            np.max(x, axis=self.axis, keepdims=True) -
            np.min(x, axis=self.axis, keepdims=True))

    def minmax(self, x):
        self.attributes = {'min': np.min(x), 'max': np.max(x)}
