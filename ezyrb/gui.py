...
class Gui(object):
	...
	def _start_ezyrb_offline(self):
		...
		self.label_new_mu.configure(
			text='New parameter value ' +
			str(self.ezyrb_handler.cvt_handler.mu_values[:, -1])
		)
	...
	def _start_ezyrb_online(self):
		...
		online_handler.run()
		online_handler.write_file(
			self.outfilename.get(), self.parsing_file_path.get()
		)
	...
	def _chose_parsing_file(self):
		...
		Up to now, you can select only .npy file.
		"""
		filename_parsing_file = askopenfilename(
			filetypes=[("VTK file", '*.vtk'), ("Matlab file", '*.mat'),
					   ('All', '*')]
		)
	...
	def main(self):
		...
		tkinter.Label(
			offline_frame, text="OFFLINE", bg='#c1d0f0', font=("Arial", 20)
		).pack()
		...
		# Buttons 1
		tkinter.Label(
			text_input_frame, text="Path and prefix"
		).grid(
			row=0, column=0
		)
		tkinter.Entry(
			text_input_frame, bd=5, textvariable=self.namefile_prefix
		).grid(
			row=0, column=1
		)
		...
		# Button 2
		tkinter.Label(
			text_input_frame, text="Output of interest"
		).grid(
			row=2, column=0
		)
		tkinter.Entry(
			text_input_frame, bd=5, textvariable=self.output_name
		).grid(
			row=2, column=1
		)

		tkinter.Label(
			text_input_frame, text="Output is a"
		).grid(
		...
		label_weights = tkinter.Label(
			text_input_frame, text="Weight name"
		).grid(
			row=1, column=0
		)
		tkinter.Entry(
			text_input_frame, bd=5, textvariable=self.weights_name
		).grid(
			row=1, column=1
		)
		...
		tkinter.Label(
			text_input_frame, text="Select file format"
		).grid(
			row=4, column=0, pady=2
		)

		tkinter.Radiobutton(
			format_frame, text=".vtk", variable=self.file_format, value='.vtk'
		).pack(side=tkinter.LEFT)
		tkinter.Radiobutton(
			format_frame, text=".mat", variable=self.file_format, value='.mat'
		).pack(side=tkinter.RIGHT)
		...
		tkinter.Label(
			online_frame, text="ONLINE", bg='#80ff80', font=("Arial", 20)
		).pack()
		...
		tkinter.Button(
			text_input_online_frame,
			text="Pick pod basis",
			command=self._chose_basis_file
		).grid(
			row=1, column=0
		)
		...
		tkinter.Label(
			text_input_online_frame, text="Output of interest"
		).grid(
			row=2, column=0
		)
		tkinter.Entry(
			text_input_online_frame, bd=5, textvariable=self.output_name
		).grid(
			row=2, column=1
		)

		tkinter.Label(
			text_input_online_frame, text="Output is a"
		).grid(
			row=3, column=0, pady=2
		)
		...
		tkinter.Label(
			text_input_online_frame, text="Select file format"
		).grid(
			row=4, column=0, pady=2
		)
		tkinter.Radiobutton(
		...
		tkinter.Button(
			...
			command=self._chose_parsing_file
		).grid(
			row=5, column=0
		)
		...
		tkinter.Label(
			text_input_online_frame, text="New parameter"
		).grid(
			row=6, column=0
		)
		tkinter.Entry(
			text_input_online_frame, bd=5, textvariable=self.new_mu
		).grid(
			row=6, column=1
		)

		tkinter.Label(
			text_input_online_frame, text="Output file name"
		).grid(
			row=7, column=0
		)
		tkinter.Entry(
			text_input_online_frame, bd=5, textvariable=self.outfilename
		).grid(
			row=7, column=1
		)
