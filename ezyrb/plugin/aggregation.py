...
class Aggregation(Plugin):
    ...
    def __init__(self, fit_function=None, predict_function=Linear()):
        ...
        super().__init__()
        logger.debug(
            "Initializing Aggregation with fit_function=%s, "
            "predict_function=%s",
    ...
    def _optimize_sigma(self, mrom, sigma_range=[1e-5, 1e-2]):
        ...
        def obj_func(sigma):
            ...
            g_sigma = self._compute_validation_weights(
                mrom, sigma, normalized=True
            )
            mrom_prediction = np.zeros_like(
                mrom.validation_full_database.snapshots_matrix
            )
            for i, rom in enumerate(mrom.roms):
                mrom_prediction += (
                    g_sigma[i, ...]
                    * mrom.roms[rom]
                    .predict(mrom.validation_full_database)
                    .snapshots_matrix
                )
            test_error = np.mean(
                np.linalg.norm(
                ...
                / np.linalg.norm(
                    mrom.validation_full_database.snapshots_matrix, axis=1
                )
            )
            return test_error
    ...
    def _compute_validation_weights(self, mrom, sigma, normalized=False):
        ...
        for name, rom in mrom.roms.items():
            validation_predicted[name] = rom.predict(
                rom.validation_full_database
            ).snapshots_matrix
        ...
        g_tensor = self._check_sum_gaussians(
            mrom, g_tensor.sum(axis=0), g_tensor
        )
    ...
    def fit_postprocessing(self, mrom):
        ...
        for i in range(rom.validation_full_database.snapshots_matrix.shape[0]):
            ...
            snapshot_input = np.hstack(
                [space, np.tile(param, (space.shape[0], 1))]
            )
        ...
        if self.fit_function is None:
            ...
            g_tensor = self._compute_validation_weights(
                mrom, optimal_sigma, normalized=False
            )

            self.predict_functions = []
            for i, rom in enumerate(mrom.roms.values()):
                g_ = g_tensor[i, ...].reshape(
                    space.shape[0] * params.shape[0], -1
                )
        ...
        # directly fit a regression to minimize the discrepancy between the aggregation and the snapshots
            ...
            self.fit_function.fit(
                input_, snaps.reshape(space.shape[0] * params.shape[0], 1)
            )
    ...
    def predict_postprocessing(self, mrom):
        ...
        if self.fit_function is None and self.predict_function is not None:
            ...
            gaussians_test = gaussians_test.reshape(
                len(mrom.roms), db.parameters_matrix.shape[0], space.shape[0]
            )
            # normalize weights
        ...
        prediction = np.sum(
            [
                mrom.weights_predict[k]
                * mrom.multi_predict_database[k].snapshots_matrix
                for k in list(mrom.roms.keys())
            ],
            axis=0,
        )

        mrom.predict_full_database = Database(
            mrom.predict_full_database.parameters_matrix, prediction
        )
