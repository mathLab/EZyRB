...
class Parameter:
    ...
    def __init__(self, values):
        ...
        """
        logger.debug(
            "Initializing Parameter with values type: %s", type(values)
        )
        if isinstance(values, Parameter):
            ...
            self.values = values
            logger.debug(
                "Created Parameter with shape: %s", np.asarray(values).shape
            )
